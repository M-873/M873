-- 🎯 **REAL OTP SYSTEM FOR OWNER LOGIN**
-- This script creates a complete OTP system for secure owner authentication
...
BEGIN
  ...
  -- Store the OTP (upsert to handle existing emails)
...
BEGIN
  ...
  -- Check if OTP matches and mark as used
...
-- Allow users to update their own OTP
CREATE POLICY "Users can update their own OTP" ON public.owner_otps
  FOR UPDATE USING (email = auth.jwt() ->> 'email');

-- Create email logs table for tracking (optional)
...
-- ✅ **SUCCESS MESSAGE**
SELECT '✅ Real OTP system created successfully!' as status;
SELECT '📧 OTP emails will be sent to: mahfuzulislam873@gmail.com' as info;
SELECT '⏰ OTPs expire in 10 minutes' as expiration;
SELECT '🔒 Secure owner login ready!' as security;