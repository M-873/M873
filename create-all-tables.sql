...
CREATE POLICY IF NOT EXISTS "Owners can view all profiles" ON public.profiles FOR ALL USING (public.has_role(auth.uid(), 'owner'));

-- Create RLS policies for user_roles table
CREATE POLICY IF NOT EXISTS "Users can view own role" ON public.user_roles FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY IF NOT EXISTS "Owners can manage roles" ON public.user_roles FOR ALL USING (public.has_role(auth.uid(), 'owner'));
