-- Final fixed SQL script - handles existing function conflicts
-- Comprehensive fix for all missing tables and RLS policies

-- Step 1: Drop existing has_role function if it exists
DROP FUNCTION IF EXISTS public.has_role(UUID, TEXT);

-- Step 2: Create the has_role function with consistent parameter names
CREATE OR REPLACE FUNCTION public.has_role(p_user_id UUID, p_required_role TEXT)
RETURNS BOOLEAN AS $$
BEGIN
    ...
    RETURN EXISTS (
        ...
        AND user_roles.role = p_required_role
    );
END;
...
-- Step 6: Create RLS policies (now the has_role function exists)
...
-- Step 8: Create owner role for your email
